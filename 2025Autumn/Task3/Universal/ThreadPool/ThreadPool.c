...
void *_ThreadPool_Worker(void *arg)
{
  ...
  while (1)
  {
    ...
    while (!pool->stop && pool->taskQueue->functions->empty(pool->taskQueue))
    {
      // 等待任务到来
    ...
    if (taskRst.success)
    {
      ...
      if (pthread_mutex_lock(&pool->queueMutex) == 0) {
        pool->activeTaskCount--;
        if (pool->activeTaskCount == 0 && pool->taskQueue->functions->empty(pool->taskQueue))
...
void ThreadPool_Wait(ThreadPool threadPool)
{
  ...
  _ThreadPool pool = (_ThreadPool)threadPool;
  pthread_mutex_lock(&pool->queueMutex);
  while (pool->activeTaskCount > 0 || !pool->taskQueue->functions->empty(pool->taskQueue))
  {
