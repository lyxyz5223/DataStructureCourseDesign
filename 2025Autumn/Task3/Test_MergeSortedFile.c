...
int main()
{
  ...
  while (1) // 无限循环，直到找不到下一个分割文件
  {
    ...
    // 提交排序任务到线程池
    String sortingFilePath = String_Create(strApi->toCString(partFilePath), 0, 1);
    printf("已提交小文件排序任务: %s\n", strApi->toCString(partFilePath));
    partIndex++;
    // 记录小文件路径
    arrApi->append(arrSmallFilePathsString, (UniversalData)sortingFilePath);                 // 保存的是指针，String本身也是指针
    arrApi->append(arrSmallFilePathsPtr, (UniversalData)strApi->toCString(sortingFilePath)); // 保存的是C字符串指针
  }
  // 等待所有排序任务完成
  ...
  ull bufferSize = 4 * 1024 * 1024;      // 4MB缓冲区
