 ...
 * 创建一个新的迭代器
 */
_Iterator _Linklist_Iterator_Create(Node *node)
{
...
_Iterator _Linklist_Iterator_Create_Begin(Linklist *list)
{
  if (!list)
    return NULL;
  return _Linklist_Iterator_Create(list->head->first);
}
/**
 * 内部函数
 * 创建一个指向结束的迭代器
 ...
 */
_Iterator _Linklist_Iterator_Create_End()
{
  return _Linklist_Iterator_Create(NULL);
}
...
Iterator Linklist_Iterator(Linklist *list, ull *index)
{
  ...
  for (ull i = 0; i < (*index); i++)
    current = current->next;
  (*index)++;
  _Iterator iter = _Linklist_Iterator_Create(current);
...
Iterator Linklist_End(Linklist *list)
{
  if (!list)
    return NULL;
  return (Iterator)_Linklist_Iterator_Create_End(list);
}
...
Linklist *Linklist_Create(UniversalData dataSet[], ull length)
{
  ...
  list->iteratorEnd = _Linklist_Iterator_Create_End(list);
