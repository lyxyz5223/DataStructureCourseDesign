#include <stdio.h>
#include <string.h>
#include "Universal/Set/ArraySet.h"
...
{
  // ull len = set->functions->length(set);
  // printf("{ ");
  // for (ull i = 0; i < len; i++)
  // {
  //   if (i == len - 1)
  //     printf("%s ", (char *)set->functions->get(set, i));
  //   else
  //     printf("%s, ", (char *)set->functions->get(set, i));
  // }
  // printf("}\n");
  ...
  for (; !iter->equals(iter, set->functions->end(set)); iter->next(iter))
  {
...
int main()
{
  ...
  // 创建集合
  Set *setA = ArraySet_Create((UniversalData *)dataA, 3, universalDataStrcmp);
  Set *setB = ArraySet_Create((UniversalData *)dataB, 4, universalDataStrcmp);
