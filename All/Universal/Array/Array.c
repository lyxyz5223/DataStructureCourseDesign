...
lboolean Array_InsertBeforeIterator(Array* array, Iterator iterator, UniversalData data)
{
  ...
  ull index = iter->currentIndex;
  iter->currentIndex++; // 插入后迭代器后移
  return array->functions->insert(array, index, data);
}
...
lboolean Array_ReplaceByIterator(Array* array, Iterator iterator, UniversalData data)
{
  ...
  _Iterator iter = (_Iterator)iterator;
  ull index = iter->currentIndex;
  return array->functions->replace(array, index, data);
}
...
lboolean Array_RemoveByIterator(Array* array, Iterator iterator)
{
  ...
  _Iterator iter = (_Iterator)iterator;
  ull index = iter->currentIndex;
  return array->functions->remove(array, index);
}
...
Iterator Array_Begin(Array *array)
{
  if (!array)
    return NULL;
  _Array *arr = (_Array *)array;
  if (array->functions->length(array) == 0)     // 如果索引超出范围，
    return (Iterator)arr->iteratorEnd;          // 返回尾部迭代器
