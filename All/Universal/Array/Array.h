...
typedef struct ArrayApi
{
   ...
   * @note 可能导致数组重新分配内存
   */
  lboolean (*insertBeforeIterator)(Array *array, Iterator iterator, UniversalData data);
  /**
   * 在数组末尾添加一个元素
   * @param array 数组指针
   * @param data 元素
   ...
   * @param iterator 迭代器
   * @return true成功，false失败
   * @note 删除操作会导致迭代器后移一个元素
   */
  lboolean (*removeByIterator)(Array *array, Iterator iterator);
  /**
   * 替换index位置的元素
   * @param array 数组指针
   * @param index 索引
   ...
   * @return true成功，false失败
   */
  lboolean (*replaceByIterator)(Array *array, Iterator iterator, UniversalData data);
  /**
   * 获取最后一个元素
   * @param array 数组指针
   * @return 元素副本
...
extern "C"
{
  ...
  lboolean Array_InsertBeforeIterator(Array *array, Iterator iterator, UniversalData data);
  lboolean Array_Append(Array *array, UniversalData data);
  lboolean Array_Replace(Array *array, ull index, UniversalData data);
  lboolean Array_ReplaceByIterator(Array *array, Iterator iterator, UniversalData data);
  lboolean Array_Remove(Array *array, ull index);
  lboolean Array_RemoveByIterator(Array *array, Iterator iterator);
